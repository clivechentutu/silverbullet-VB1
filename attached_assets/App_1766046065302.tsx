...
import { ArrowRight, Sparkles, Globe, AlertCircle, BarChart3, Target, LayoutDashboard } from 'lucide-react';
...
const LandingPage: React.FC = () => {
  ...
  const executeAnalysis = async () => {
    ...
    try {
      const result = await generateMarketIntelligence(url, selectedScenarios);
  ...
  return (
    <>
      ...
      <main className="relative pt-32 pb-20 px-6 max-w-7xl mx-auto flex flex-col items-center min-h-screen">
        ...
        <div className={`w-full max-w-4xl mx-auto text-center transition-all duration-700 ${appState === AppState.RESULTS ? 'hidden' : 'block'}`}>
          ...
          <div className={`relative max-w-2xl mx-auto transition-all duration-500 ${appState !== AppState.IDLE ? 'scale-100' : 'hover:scale-[1.01]'}`}>
            ...
            {errorMsg && (
              <div className="absolute top-full left-0 mt-2 text-red-400 text-sm flex items-center gap-1 animate-fade-in-up">
        ...
        {appState === AppState.ANALYZING && (
          <div className="mt-20 flex flex-col items-center animate-fade-in-up">
            ...
            <h3 className="mt-6 text-xl font-medium text-slate-200">Orchestrating AI Agents...</h3>
        ...
        {appState === AppState.RESULTS && analysisResult && (
          <div className="w-full max-w-5xl animate-fade-in-up mt-8">
            <button onClick={reset} className="mb-8 text-sm text-slate-400 hover:text-white flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-8">
                <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-8 backdrop-blur-sm">
                  ...
                  <p className="text-slate-300 leading-relaxed text-lg">{analysisResult.summary}</p>
                </div>
              ...
              {/* Sidebar: Competitors & Battle Cards */}
                ...
                <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6 backdrop-blur-sm">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {analysisResult.competitors.map((comp, i) => (
                      <span key={i} className="px-3 py-1 bg-slate-800 text-slate-200 rounded-full text-sm border border-slate-700">
                        {comp}
                      </span>
                    ))}
                  </div>
        ...
        {appState === AppState.ERROR && (
           <div className="mt-20 text-center animate-fade-in-up">
             ...
             <p className="text-slate-400 mb-8 max-w-md mx-auto">{errorMsg}</p>
             <button 
               onClick={reset}
      ...
      <footer className="border-t border-slate-900 py-8 bg-slate-950/50 backdrop-blur-sm mt-auto">
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center text-slate-600 text-sm">
...
const App: React.FC = () => {
  // Global view state to switch between Landing Page and Workbench for preview
  const [currentView, setCurrentView] = useState<'LANDING' | 'WORKBENCH'>('LANDING');

  return (
    <div className="min-h-screen bg-[#020617] text-white selection:bg-brand-500/30 selection:text-brand-200">
      
      {/* Dev Navigation Tabs */}
      <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[100] flex gap-1 p-1 bg-slate-900/90 border border-slate-700/50 rounded-full backdrop-blur shadow-xl">
        <button 
          onClick={() => setCurrentView('LANDING')}
          className={`px-4 py-1.5 rounded-full text-xs font-medium transition-all ${currentView === 'LANDING' ? 'bg-brand-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}
        >
          Landing
        </button>
        <button 
          onClick={() => setCurrentView('WORKBENCH')}
          className={`px-4 py-1.5 rounded-full text-xs font-medium flex items-center gap-1 transition-all ${currentView === 'WORKBENCH' ? 'bg-brand-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}
        >
          <LayoutDashboard size={12} /> Workbench
        </button>
      </div>

      {currentView === 'LANDING' ? <LandingPage /> : <Workbench />}
    </div>
  );
};
